...
| `kanban-task-engine-one-page.svg` | one-page README embed 및 full-size/raw view용 SVG 렌더링 |
| `kanban-task-engine-one-page.html` | one-page 로컬 브라우저 확인용 HTML 렌더링 |
| `kanban-use-case.drawio` | Home Assisted use-case draw.io 편집용 소스 (mxGraph XML) |
| `kanban-use-case.svg` | Home Assisted use-case README embed 및 full-size/raw view용 SVG 렌더링 |
| `kanban-use-case.html` | Home Assisted use-case 로컬 브라우저 확인용 HTML 렌더링 |
...
2. 수정 대상에 맞는 source 파일 열기: `kanban-task-engine-one-page.drawio` 또는 `kanban-use-case.drawio`
3. 수정 후 **File → Export as → SVG** 선택
4. 대응하는 SVG 렌더(`kanban-task-engine-one-page.svg` 또는 `kanban-use-case.svg`) 덮어쓰기
5. 필요하면 대응하는 HTML 렌더(`kanban-task-engine-one-page.html` 또는 `kanban-use-case.html`)도 같은 의미로 맞추기
6. 아래 GitHub README SVG Contract의 hardening 항목을 확인
7. 검증: `python3 scripts/verify-docs.py`
...
- `kanban-task-engine-one-page.{drawio,svg,html}`은 one-page source/render/browser 확인 세트입니다. architecture 의미나 layout이 바뀌면 drawio와 HTML을 함께 맞추고, SVG production render를 다시 생성하거나 수동 hardening합니다.
- `kanban-use-case.{drawio,svg,html}`은 Home Assisted use-case source/render/browser 확인 세트입니다. 시나리오 카드, 상태 컬럼, CLI/session ID 의미가 바뀌면 drawio와 HTML을 함께 맞추고, SVG production render를 다시 생성하거나 수동 hardening합니다.
