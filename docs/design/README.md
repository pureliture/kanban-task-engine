...
| `kanban-use-case.drawio` | AI CLI 실행 연동 use-case draw.io 편집용 소스 (mxGraph XML) |
...
- `kanban-use-case.{drawio,svg,html}`은 AI CLI 실행 연동 use-case source/render/browser 확인 세트입니다. 시나리오 카드, 상태 컬럼, CLI/session ID 의미가 바뀌면 drawio와 HTML을 함께 맞추고, SVG production render를 다시 생성하거나 수동 hardening합니다.
