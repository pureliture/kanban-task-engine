...
| `kanban-task-engine-one-page.svg` | full-size/raw view용 one-page SVG 렌더링 |
| `kanban-task-engine-architecture-overview.svg` | GitHub README embed용 compact SVG 렌더링 (수동 작성, 외부 asset 없음) |
...
- README embed용 compact SVG는 900px 표시 기준 본문 텍스트가 8px 이상, 주요 zone/title 텍스트가 12px 이상으로 보이도록 설계합니다.
...
- `kanban-task-engine-architecture-overview.svg`는 README 가독성을 위한 수동 compact render입니다. one-page SVG의 핵심 의미(Vault, Engine, External, executor, Work mode 제한, status contract)와 drift되면 안 됩니다.
