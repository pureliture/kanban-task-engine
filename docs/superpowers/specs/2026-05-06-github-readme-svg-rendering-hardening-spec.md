...
**Implementation Note:** 2026-05-06 implementation restores the README embed to the original one-page SVG, keeps a compact SVG only as an auxiliary asset, adds fixture-based docs verifier tests, splits the Python verifier into small `scripts/verify_docs/*` modules, and passes local build/test/docs/eval gates.
**Correction Note:** A previous implementation briefly embedded the compact overview in `README.md`; that was rejected because the canonical README preview must be `kanban-task-engine-one-page.svg`.
...
- `README.md` 전체 리라이트. 보조 compact overview asset은 허용하지만 canonical README embed를 원본 `kanban-task-engine-one-page.svg`에서 다른 이미지로 바꾸는 것은 목표 범위 밖이다.
...
이번 이슈의 본질은 asset이 README embed contract를 충족하지 못하는 것이다. 따라서 Option C를 기본 결정으로 삼는다. README canonical preview는 원본 one-page SVG로 유지한다. compact overview SVG를 만들 수는 있지만, README 기본 embed를 대체하지 않고 보조 자료로만 둔다. 어떤 경로를 택하든 one-page SVG 자체는 self-contained raw SVG로 정상 렌더링되어야 한다.
...
9. Estimated README display font-size applies to alternate compact README assets. The one-page SVG is the canonical README/full-size/raw target and must pass self-contained, background, forbidden-reference, accessibility, and runtime-truth gates.
...
The README architecture image must embed `docs/design/kanban-task-engine-one-page.svg`. `docs/design/kanban-task-engine-architecture-overview.svg` may exist as an auxiliary compact overview, but it must not replace the canonical one-page README preview unless the owner explicitly requests a compact preview.
...
| High-density one-page asset used as README preview | UX/documentation debt | 4 | 3 | 3 | 21 | Keep one-page as canonical preview, harden its self-contained SVG contract, and rely on full-size click path for dense details. |
...
- [ ] README embeds `docs/design/kanban-task-engine-one-page.svg` directly; compact overview remains auxiliary unless explicitly requested.
