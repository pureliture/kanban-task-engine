...
describe('prompt-builder', () => {
  const baseTask: CanonicalTaskModel = {
    ...
    automation: { policy_id: 'default', on_enter: ['RUNNING'], on_exit: [], execution_profile: 'standard', workspace: 'ws' },
    sync: { last_synced_at: '2026-04-16T10:00:00Z', last_source: 'local' },
  };

  it('includes task summary in prompt', () => {
    const prompt = buildExecutionPrompt(baseTask, { from: 'READY', to: 'RUNNING' });
  ...
  it('includes context when enabled', () => {
    const prompt = buildExecutionPrompt(baseTask, { from: 'READY', to: 'RUNNING' }, { includeContext: true });
  ...
  it('excludes context when disabled', () => {
    const prompt = buildExecutionPrompt(baseTask, { from: 'READY', to: 'RUNNING' }, { includeContext: false });
  ...
  it('provides transition-specific instructions', () => {
    const prompt = buildExecutionPrompt(baseTask, { from: 'RUNNING', to: 'REVIEW' });
    expect(prompt).toContain('review');
  });

  it('falls back to generic instruction for unknown transitions', () => {
    const task = { ...baseTask };
    const prompt = buildExecutionPrompt(task, { from: 'DONE' as NormalizedStatus, to: 'RUNNING' as NormalizedStatus });
