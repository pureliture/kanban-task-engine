...
- **Issue Status Transitions**: `TODO ⇄ READY → RUNNING → REVIEW ⇄ RUNNING`, `RUNNING → FAILED → READY`, `REVIEW → DONE`. `READY → TODO`(되돌리기)와 `REVIEW → RUNNING`(재실행) 포함 총 8개 전이.
- **Executors**: `claude-code-executor`, `codex-runner` — 각각 별도 worktree에서 실행.
</details>
...
`validate-only` mode는 recipe `modules`와 `policy`를 로드하되 mutation 없이 schema/policy validation만 수행합니다. `recipes/validate-only.yaml`로 활성화합니다.
...
Issue status 전이 규칙 (`packages/schema/src/status.ts` 기준):

| 전이 | 방향 | 설명 |
|---|---|---|
| `TODO → READY` | 전진 | 실행 준비 완료 |
| `READY → TODO` | 후퇴 | 준비 취소 (되돌리기) |
| `READY → RUNNING` | 전진 | 실행 시작 (`--execute`) |
| `RUNNING → REVIEW` | 전진 | agent exit 0 + file change |
| `RUNNING → FAILED` | 전진 | agent exit non-0 또는 no-change |
| `REVIEW → DONE` | 전진 | `kanban approve` |
| `REVIEW → RUNNING` | 후퇴 | `kanban retry` (재실행 트리거) |
| `FAILED → READY` | 후퇴 | `kanban retry` 후 재시도 준비 |
